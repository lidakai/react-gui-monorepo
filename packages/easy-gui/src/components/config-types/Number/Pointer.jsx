  ...
  <svg width={46} height={15} className={styles.svg}>
    <g transform="translate(2 3)">
      <path
        ...
        style={{ strokeWidth: 2 }}
      />
      <path fillRule="evenodd" d="M 15 4.5L 15 2L 11.5 5.5L 15 9L 15 6.5L 31 6.5L 31 9L 34.5 5.5L 31 2L 31 4.5Z" />
    </g>
  </svg>
);
...
export default function Pointer({ defaultX, defaultY, onMove }) {
  ...
  useEffect(() => {
    ...
    const handleUpdateValue = (e) => {
      ...
      pointerRef.current.style = `transform: translate3d(${newPos.left}px,${newPos.top}px,0px)`;
    };

    pointerRef.current.className = styles.arrow;
    pointerRef.current.style = `transform: translate3d(${Math.floor(newPos.left)}px,${Math.floor(newPos.top)}px,0)`;
  ...
  return createPortal(arrow, pointerRef.current);
}
