      ...
      {data.map((d) => {
        return (
          <div
            key={d.value}
            onClick={() => {
              setSelectedKey(d.value);
              onChange(d.value);
            }}
            className={styles.imgCardBox}>
            <div className={styles.imgCardItem}>
              <Image
                preview={false}
                placeholder={IconLoading}
                fallback={IconError}
                className={classNames(styles.cardImage, { [styles.cardImageSelected]: selectedKey === d.value })}
                src={d.cover || IconError}
              />
            </div>
            <span className={classNames(styles.label, 'label', { [styles.labelSelected]: selectedKey === d.value })}>{d.name}</span>
          </div>
        );
      })}
    </div>
